...
## 로드밸런싱
여러 서버에 걸쳐 트래픽을 분산시키는 네트워크 기술로, 로드밸런서가 들어오는 요청을 여러 서버로 분산시킨다.
* **라운드로빈**
  * 순차적으로 각 서버에 요청을 할당하는 방식
  * 각 서버의 스펙이 균등하고 연결이 오래 지속되지 않는경우에 적합하다.
* **가중 라운드로빈**
  * 각 서버에 가중치를 부여하여, 높은 성능의 서버에 더 많은 요청을 할당하는 방식
  * 서버간의 스펙차이가 클때 유용하다.
* **IP 해시**
  * 클라이언트 IP주소를 해싱하여, 특정 서버에 요청을 고정하는 방식
  * 세션유지가 필요한 어플리케이션에서 유용하다.
* **최소 연결**
  * 요청이 들어온 시점에 가장 적은 연결 상태를 보이는 서버에 우선적으로 트래픽을 할당하는 방식
* **최소 응답 시간**
  * 서버의 현재 연결 상태와 응답 시간을 모두 고려하여, 가장 짧은 응답 시간을 보내는 서버로 트래픽을 할당하는 방식
...
* **L4**
  ...
  * IP주소와 포트번호로 트래픽을 관리
  * 빠르지만 활용할 데이터가 적어 제한적이다.
* **L7**
  * 응용계층 (Application Layer)
  * HTTP 헤더, URL, 쿠키 등 애플리케이션 계층의 데이터를 분석하여 트래픽을 관리
  * 느리지만 활용할 데이터가 많아 유연하다.
